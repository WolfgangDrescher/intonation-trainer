        ...
        <div @click="openDropdown" class="relative">
            ...
            <div v-if="isOpen" ref="dropdownOptions" class="absolute shadow-sm border border-gray-200 mt-2 top-full bg-white z-40 w-full left-0 rounded max-h-80 overflow-y-auto">
                <div class="flex flex-col w-full">
                    ...
                    <FormDropdownOption v-for="(option, index) in filteredOptions" :key="index" :value="option.value ?? option" :text="option.text ?? option" :selected="optionIsSelected(option.value)" @toggleOption="toggleOption" />
                </div>
            </div>
        </div>
    </FormGroup>
